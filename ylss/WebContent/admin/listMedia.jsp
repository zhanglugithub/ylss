		...
		<div class="main-content">
			...
			<div>
				...
				<div class="panel panel-default">
					<!-- start content -->
					<div align="right">
						<a class="btn btn-success" href="mediaDetail.jsp">添加</a>
					</div>
					<table class="table table-model-2 table-hover">
						...
						<tbody>
							<c:forEach var="media" items="${listMedia}">
								<tr>
									<td>${media.id}</td>
									<td>
									...
									<td>
										<div class="form-group">
											<button onclick="window.open('updateMedia.do?review=true&id=${media.id}')"  class="form-group">修改</button>
										</div>
									</td>
								</tr>
							</c:forEach>
						...
						<c:forEach var="pageNum" begin="1" end="${pageNo/10+1}">
							<c:if test="${pageNum<6}">
								<a href="listMedia.do?pageSize=10&pageNo=${pageNum}"> ${pageNum}</a>
							</c:if>
						</c:forEach>
				</div>
