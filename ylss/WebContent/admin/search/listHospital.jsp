			...
			<div>
				...
				<c:if test="${hospitals.code==1}">
					<div class="panel panel-default">
						<!-- start content -->
						<div align="right">
							<a href="search/HospitalInfo.jsp" class="btn btn-success"
								style="font-size: 20px;">添加医院</a>
						</div>
						<table class="table table-model-2 table-hover">
							...
							<tbody>
								<c:forEach var="hospital" items="${hospitals.hospitals}">
									<tr>
										<td>${hospital.getHoId()}</td>
										<td>${hospital.getHospital()}</td>
										<td>${hospital.getAddTime()}</td>
										<td><a
											href="detailHospital.do?hoId=${hospital.getHoId()}"
						...
						<div class="title-env">
							<c:forEach var="pageNo" begin="1" end="${hospitals.pageCount}">
								<a href="getHospital.do?pageSize=10&pageNo=${pageNo}">
									${pageNo} </a>
							</c:forEach>
						</div>
					</div>
