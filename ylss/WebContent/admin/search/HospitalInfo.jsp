	...
	<div class="page-container">
		...
		<div class="main-content">
			...
			<div class="page-title">
				...
				<div class="title-env">
					<h1 class="title">添加或修改医院</h1>
					<p class="description">医院</p>
				</div>
			</div>

			<div>
				<c:if test="${hospitals.code==0}">
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<c:if test="${hospitals.code==1}">
							<c:if test="${hospitals.msg!=null}">
								<p style="font-size: 30px; color: black;">${hospitals.msg}</p>
							</c:if>
							<c:if test="${hospitals.msg==null}">
								<form class="validate"
									action="detailHospital.do?hoId=${hospitals.hospital.getHoId()}&addTime=${hospitals.hospital.getAddTime()}"
									method="post">
									<p>
										医院ID：<a class="form-control">${hospitals.hospital.getHoId()}</a>
									</p>
									<p>
										创建时间： <a class="form-control">${hospitals.hospital.getAddTime()}</a>
									</p>
									<input type="hidden" name="uid"
										value="${hospitals.hospital.getUid()}" required="required">
									<p>
										医院名称： <input name="hospital"
											value="${hospitals.hospital.getHospital()}" type="text"
											class="form-control" required="required">
									</p>
									<p>
										是否删除：
										<c:if test="${hospitals.hospital.getIsDel()==1}">
											<input type="radio" value="1" name="isDel" checked="checked">未删除
											<input type="radio" value="0" name="isDel">删除
										</c:if>
										<c:if test="${hospitals.hospital.getIsDel()==0}">
						...
						<c:if test="${hospitals.code==null}">
							<form action="setHospital.do" method="post">
								<p>
									医院名称： <input name="hospital" type="text">
								</p>
								<p>
									<button type="submit" class="btn btn-success">添加医院</button>
								</p>
							</form>
						</c:if>
					</div>
