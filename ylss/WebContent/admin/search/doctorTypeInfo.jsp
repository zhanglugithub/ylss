	...
	<div class="page-container">
		...
		<div class="main-content">
			...
			<div class="page-title">
				...
				<div class="title-env">
					<h1 class="title">添加或修改类别</h1>
					<p class="description">医生类别</p>
				</div>
			</div>

			<div>
				<c:if test="${detailType.code==0}">
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<c:if test="${detailType.code==1}">
							<c:if test="${detailType.msg!=null}">
								<p style="font-size: 30px; color: black;">${detailType.msg}</p>
							</c:if>
							<c:if test="${detailType.msg==null}">
								<form class="validate"
									action="detailDoctorType.do?dtId=${detailType.doctorType.getDtId()}&createTime=${detailType.doctorType.getCreateTime()}"
									method="post">
									<p>
										类别ID：<a class="form-control">${detailType.doctorType.getDtId()}</a>
									</p>
									<p>
										创建时间： <a class="form-control">${detailType.doctorType.getCreateTime()}</a>
									</p>
									<input type="hidden" name="uid"
										value="${detailType.doctorType.getUid()}" required="required">
									<p>
										类别名称： <input name="doctorType"
											value="${detailType.doctorType.getDoctorType()}" type="text"
											class="form-control" required="required">
									</p>
									<p>
										是否删除：
										<c:if test="${detailType.doctorType.getIsDel()==1}">
											<input type="radio" value="1" name="isDel" checked="checked">未删除
											<input type="radio" value="0" name="isDel">删除
										</c:if>
										<c:if test="${detailType.doctorType.getIsDel()==0}">
						...
						<c:if test="${detailType.code==null}">
							<form action="setDoctorType.do" method="post">
								<p>
									类别名称： <input name="doctorType" type="text">
								</p>
								<p>
